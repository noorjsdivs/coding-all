class BlogPost
  attr_reader :slug, :title, :excerpt, :content, :image, :date, :read_time, :category, :author, :author_image

  def initialize(slug:, title:, excerpt:, content:, image:, date:, read_time:, category:, author:, author_image:)
    @slug = slug
    @title = title
    @excerpt = excerpt
    @content = content
  ...
  def self.related_posts(slug, category, limit = 3)
    related = all.select { |post| post.slug != slug && post.category == category }
    
    if related.empty?
      all.select { |post| post.slug != slug }.take(limit)
    else
      related.take(limit)
    end
  end
